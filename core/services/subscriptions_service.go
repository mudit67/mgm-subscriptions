...
func (s *SubscriptionService) UpsertSubscription(ctx context.Context, req *models.CreateSubscriptionRequest) (*models.Subscription, error) {
	// Get plan details first
	plan, err := s.planRepo.GetByID(ctx, req.PlanID)
	if err != nil {
		return nil, errors.New("plan not found")
	...
	if existingSub != nil {
		// Check if existing subscription is expired and update status
		if existingSub.Status == models.StatusActive && time.Now().After(existingSub.ExpiresAt) {
			existingSub.Status = models.StatusExpired
		}

		// Determine action based on current status
		switch existingSub.Status {
		case models.StatusActive:
			if existingSub.PlanID.Hex() == req.PlanID.Hex() {
				action = "renewed"
			} else {
				action = "updated"
			}
		case models.StatusCancelled:
			action = "renewed"
		case models.StatusExpired, models.StatusInactive:
			action = "created"
		}

		// Use upsert to handle all cases
		if err := s.subscriptionRepo.Upsert(ctx, subscription); err != nil {
			return nil, err
		}

		log.Printf("Subscription %s for user %s", action, req.UserID)
	} else {
		// Create new subscription
		if err := s.subscriptionRepo.Create(ctx, subscription); err != nil {
			return nil, err
